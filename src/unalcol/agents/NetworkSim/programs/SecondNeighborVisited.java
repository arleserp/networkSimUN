 ...
 * @author My-Macintosh
 */
public class SecondNeighborVisited implements AgentProgram {
       float pf;

    public SecondNeighborVisited(float pf) {
        this.pf = pf;
    ...
    public Action compute(Percept p) {
        ...
        GraphElements.MyVertex me = (GraphElements.MyVertex)p.getAttribute("currentLocation");
        ArrayList<GraphElements.MyVertex> segundosvecinos = (ArrayList<GraphElements.MyVertex>)p.getAttribute("segundosvecinos");
        ...
        if (vs.size() == 0){
            for(GraphElements.MyVertex index : segundosvecinos){
                if(index.getStatus()!="v")
                    vs.add(index);
            }
            if(vs.size() == 0){
                vs=tt;
